...
export default function StudyPlanForm({ onPlanGenerated }: StudyPlanFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!session?.user?.id) {
      toast.error(
        "You must be logged in to generate a study plan.");
      return;
    }
    ...
    try {
      ...
      if (response.success && response.plan) {
        ...
        setSubject('');
        setDate(undefined);
        toast.success(response.message || 'Study plan generated successfully');
      ...
      if (error?.response?.data?.error === 'PLAN_EXISTS') {
       ...
       toast.error(error instanceof Error ? error.message : 'Failed to generate study plan. Please try again.');
      }
    } finally {
      setIsLoading(false);
    }
