  ...
  const fetchPlans = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        toast.error(
          "Failed to fetch study plans. Please try again."
        );
        setStoredPlans([]);
        return;
      }
      ...
      toast.error("Failed to fetch study plans. Please try again."
      );
      setStoredPlans([]);
    } finally {
      setLoading(false);
  ...
  const handlePlanDelete = async (planId: string) => {
    try {
      ...
      if (response.success) {
        ...
        toast.success(response.message || "Study plan deactivated successfully."
        );
      } else {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to deactivate study plan. Please try again."
      );
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 sm:mb-0">Study Plan Generator</h1>
        <div className="flex items-center gap-4">
          <span className="text-xs sm:text-sm text-gray-100">Create and manage your study plans</span>
        </div>
      </div>
