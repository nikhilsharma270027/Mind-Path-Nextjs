    ...
    try {
      ...
      if (error && typeof error === 'object' && 'status' in error) {
        ...
        if (errorObj.status === 429 || errorObj.status === 413) {
          ...
          toast.error(`Please try again in ${minutes} minute${minutes > 1 ? 's' : ''}. Our AI service has reached its limit.`);
          return;
        }
      }
      ...
      if (error && typeof error === 'object' && 'response' in error) {
        ...
        if (responseError.response?.data?.error === 'RESOURCE_EXISTS') {
          toast.error(responseError.response.data.message || "You already have resources for this subject.");
      ...
      toast.error(error instanceof Error ? error.message : "Failed to create resources. Please try again.");
    }
  };
